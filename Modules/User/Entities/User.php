...
{
    use Authenticatable;
	protected $fillable = ['username','username2','first_name','middle_name','last_name', 'email','avatar','password']; //← only the field names inside the array can be mass-assign
    ...
    public function hasRoleIds($roleIds)
    {
        return $this->roles()->whereIn('id',$roleIds)->count() !== 0;
    }
    /**
    ...
    {
        return new UserTable($this->newQuery()->where('password','!=','nopassword')->where('id','!=',auth()->user()->id));
    }
