...
@section('content')
    <div class="container">
        @if ($cart->isEmpty())
            <h2 class="text-center" style="margin: 30px 0 0;">{{ trans('storefront::cart.your_cart_is_empty') }}</h2>
        @else
            <div style="width: 100%; float: left; margin: 30px 0;">
                <div class="box-wrapper clearfix">
                            ...
                            <table class="table">
                                ...
                                @foreach ($cart->items() as $cartItem)
                                    <tr class="cart-item">
                                        ...
                                        <td>
                                            <h5>
                                                {{--                                                        {{ route('products.show', $cartItem->product->slug) }}--}}
                                            ...
                                            @if(!empty($cartItem->custom_design_image))
                                                <a href="{{$cartItem->custom_design_image}}" target="_blank">View
                                                    Design</a>
                                            @endif
                                        </td>
                                            ...
                                            <div class="quantity pull-left clearfix">
                                                <div class="input-group-quantity pull-left clearfix">
                                                    <input type="text" name="qty" value="{{ $cartItem->qty }}"
                                                           class="input-number input-quantity pull-left {{ "qty-{$loop->index}" }}"
                                                           min="1"
                                                    ...
                                                    <span class="pull-left btn-wrapper">
                                                                ...
                                                                <button type="button" class="btn btn-number btn-minus"
                                                                        data-type="minus" {{ $cartItem->qty === 1 ? 'disabled' : '' }}> &#8211; </button>
                                                            </span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <button data-toggle="tooltip" data-placement="top"
                                                    title="{{ trans('storefront::cart.update') }}" class="btn-update"
                                                    data-id={{ encrypt($cartItem->id) }} data-quantity-field="{{ ".qty-{$loop->index}" }}">
                                                <i class="fa fa-refresh" aria-hidden="true"></i>
                                            </button>

                                            <form method="POST"
                                                  action="{{ route('cart.items.destroy', encrypt($cartItem->id)) }}"
                                                  onsubmit="return confirm('{{ trans('cart::messages.confirm') }}');">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}

                                                <button type="submit" class="cross-button" data-toggle="tooltip"
                                                        data-placement="top"
                    ...
                    <div class="cart-list-bottom">
                        <form method="POST" action="{{ route('cart.coupon.store') }}" id="coupon-apply-form"
                              class="clearfix">
                            {{ csrf_field() }}
        ...
        <div style="width: 100%; float: left;">
            <div class="order-review cart-list-sidebar">
                <div class="cart-total">
                                ...
                                @foreach ($cart->availableShippingMethods() as $name => $shippingMethod)
                                    <div class="radio">
                                        <input type="radio" name="shipping_method" class="shipping-method"
                                               value="{{ $name }}"
                                               id="{{ $name }}" {{ $cart->shippingMethod()->name() === $shippingMethod->name || $loop->first ? 'checked' : '' }}>
                                        <label for="{{ $name }}">{{ $shippingMethod->label }}</label>
                                        <span
                                            class="pull-right">{{ $shippingMethod->cost->convertToCurrentCurrency()->format() }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if ($cart->hasCoupon())
                        <span class="item-amount">
                                    {{ trans('storefront::cart.coupon') }} (<span
                                class="coupon-code">{{ $cart->coupon()->code() }}</span>)
                                    <span
                                        id="coupon-value">&#8211;{{ $cart->coupon()->value()->convertToCurrentCurrency()->format() }}</span>
                                </span>
                    @endif
                    ...
                    <span class="total">
                                {{ trans('storefront::cart.total') }}
                                    <span
                                        id="total-amount">{{ $cart->total()->convertToCurrentCurrency()->format() }}</span>
                            </span>

                    @if ($cart->hasNoAvailableShippingMethod())
                        <span
                            class="error-message text-center">{{ trans('storefront::cart.no_shipping_method_is_available') }}</span>
                    @endif

                    <a href="{{ route('checkout.create') }}"
                       class="btn btn-primary btn-checkout {{ $cart->hasNoAvailableShippingMethod() ? 'disabled' : '' }}"
                       data-loading>
